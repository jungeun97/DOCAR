  ...
  nav_2d_msgs__msg__Pose2DStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    nav_2d_msgs__msg__Pose2DStamped * data =
      (nav_2d_msgs__msg__Pose2DStamped *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!nav_2d_msgs__msg__Pose2DStamped__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          nav_2d_msgs__msg__Pose2DStamped__fini(&data[i]);
        }
        free(data);
        return false;
      }
