  ...
  nav2_msgs__msg__CostmapMetaData__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    nav2_msgs__msg__CostmapMetaData * data =
      (nav2_msgs__msg__CostmapMetaData *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!nav2_msgs__msg__CostmapMetaData__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          nav2_msgs__msg__CostmapMetaData__fini(&data[i]);
        }
        free(data);
        return false;
      }
